...
class Journey {
  ...
  isPossible() {
    const balloonLift = (this.balloons * 4.8) / 1000; //kg
    const totalWeight = this.crew * 80 + this.object.weight; //kg
    return balloonLift > totalWeight;
  }
}

let carl = new Journey('house', 2, 20622);
let house = { weight: 45000 };
let letterToEllie = { weight: 0.004536 };
console.log(carl.isPossible(house.weight));
console.log(carl.isPossible(letterToEllie.weight));
